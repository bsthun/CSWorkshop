...
func (r *Handler) HandleExamQuestionList(c *fiber.Ctx) error {
	...
	// * parse body
	body := new(payload.ExamQuestionListRequest)
	...
	questions, er := gut.Iterate(rows, func(row psql.ExamQuestionListRow) (*payload.ExamQuestionListItem, *gut.ErrorInstance) {
		return &payload.ExamQuestionListItem{
			Id:       row.Id,
