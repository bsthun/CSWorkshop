...
func (r *Handler) HandleClassEdit(c *fiber.Ctx) error {
	// * get user claims
	u := c.Locals("l").(*jwt.Token).Claims.(*common.LoginClaims)

	// * parse body
	body := new(payload.ClassEditRequest)
	...
	// * update class
	_, err = r.database.P().ClassUpdate(c.Context(), &psql.ClassUpdateParams{
		Id:           body.Id,
		Code:         body.Code,
		Name:         body.Name,
	...
	if err != nil {
		return gut.Err(false, "failed to update class", err)
	}

	// * response
	return c.JSON(response.Success(c, "successfully updated class"))
}
