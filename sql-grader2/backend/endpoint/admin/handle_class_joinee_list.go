...
func (r *Handler) HandleClassJoineeList(c *fiber.Ctx) error {
	// * get user claims
	...
	// * get class joinee list with user info
	joineeRows, err := r.database.P().ClassJoineeList(c.Context(), body.ClassId)
	if err != nil {
	...
	// * prepare response
	responsePayload := &payload.ClassJoineeListResponse{
		Joinees: joinees,
	}
